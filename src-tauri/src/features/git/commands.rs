...
/// `on_commit` est appelé quand le `HEAD` d'un worktree surveillé bouge — un commit a pu y
...
pub fn watch_metadata<R: Runtime>(
    ...
    on_commit: impl Fn(&Path, &Path) + Send + Sync + 'static,
) -> Arc<MetadataWatch> {
    MetadataWatch::new(
