import type { GitHead, WorktreeMetadata } from "@/shared/ipc";
...
export function windowTitle(active: ActiveTab | null): string {
    ...
    const branch = branchOf(active.metadata);

    // Sans branche connue — un onglet ouvert à `~`, un dossier hors dépôt, ou le temps de
    ...
    return branch === null ? `ash — ${where}` : `ash — ${where} / ${branch}`;
}

/**
 * La branche à écrire, ou `null` quand il n'y en a pas.
 *
 * Même lecture que la ligne de statut, et pour les mêmes raisons : pendant un rebase `HEAD`
 * est détaché, et c'est `operation.branch` — le `head-name` que git garde — qui dit encore
 * où l'on travaille. Un détachement sans opération s'écrit `@a1b2c3d`, court et impossible à
 * confondre avec un nom de branche.
 */
function branchOf(metadata: WorktreeMetadata | null): string | null {
    if (metadata === null) return null;
    return metadata.operation?.branch ?? headLabel(metadata.head);
}

function headLabel(head: GitHead): string {
    return head.kind === "branch" ? head.name : `@${head.commit}`;
}
