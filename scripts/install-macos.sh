...
# Le dépôt, son API et le nom du bundle : écrits une seule fois, ici.
...
fetch_latest_release() {
    local body status=0
    body="$(curl -fsSL --connect-timeout 10 --max-time 60 "$LATEST_URL" 2>/dev/null)" || status=$?
    if [ "$status" -ne 0 ]; then
...
# L'URL de l'archive attendue, prise dans la release plutôt que composée : si l'asset manque,
# la liste ne le contient pas et l'échec est franc (5) au lieu d'un 404 au téléchargement.
asset_url() {
    local body="$1" name="$2"
    local pattern="/${name//./\\.}\$"
    printf '%s\n' "$body" |
        sed -n 's/.*"browser_download_url"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' |
        grep -m 1 -e "$pattern" || true
}
...
install_release() {
    ...
    STAGING="$(mktemp -d "${install_dir}/.ash-install.XXXXXX")" || die 1 "impossible de créer un dossier de bascule dans ${install_dir}."
    [ -n "$STAGING" ] && [ -d "$STAGING" ] || die 1 "impossible de créer un dossier de bascule dans ${install_dir}."
    trap cleanup EXIT

    say "téléchargement de ${name}…"
    curl -fsSL --connect-timeout 10 --max-time 600 -o "${STAGING}/archive.zip" "$url" ||
...
main() {
    ...
    url="$(asset_url "$body" "$name")"
    [ -n "$url" ] || die 5 "la release ${tag} ne contient pas ${name}."
